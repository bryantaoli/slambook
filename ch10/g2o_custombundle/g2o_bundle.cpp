...
typedef Eigen::Map<Eigen::VectorXd> VectorRef;  // Map内存映射，参考向量
typedef Eigen::Map<const Eigen::VectorXd> ConstVectorRef;   // 常参考向量
...
void SetMinimizerOptions(std::shared_ptr<BalBlockSolver>& solver_ptr, const BundleParams& params, g2o::SparseOptimizer* optimizer)
{
    ...
    if(params.trust_region_strategy == "levenberg_marquardt"){
        solver = new g2o::OptimizationAlgorithmLevenberg(std::unique_ptr<BalBlockSolver>(solver_ptr.get()));
    }
    else if(params.trust_region_strategy == "dogleg"){
        solver = new g2o::OptimizationAlgorithmDogleg(std::unique_ptr<BalBlockSolver>(solver_ptr.get()));
    }
    else 
    {
